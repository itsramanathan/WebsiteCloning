...
A small internal Node application that turns a public, server-rendered store or B2B URL into a bounded one-to-eight-page website preview. It uses fixed trusted components, local durable files, private operator-controlled sharing, and local-only cart/quote state.
...
- The operator selects a maximum of one to eight demo pages (default five). A credible catalog can produce Home, Collection/Solutions, and captured detail routes; a lower request or smaller catalog omits later routes without inventing content. A source with no credible catalog still produces a one-page Home preview. Requested and generated counts persist in the artifact, and the requested maximum persists across Retry.
- Samples at most four same-origin source pages, six retail products or three B2B offerings, 30 safe raster images, and 25 MB of assets within a five-minute attempt. A selected secondary page that redirects off-origin is skipped and recorded as a limitation rather than discarding a credible home-page catalog.
- Saves images locally after MIME and file-signature checks. It does not hotlink source media or accept SVG.
- Renders only Home, any supported Collection/Solutions page, and the supported number of distinct reusable item-detail views with source name, imagery, conservative color hints, and captured catalog facts. A hero is rendered only from a visible hero section; metadata-only share or logo images do not invent one.
- Keeps unknown prices/specifications unknown and never invents a catalog to meet the requested maximum.
...
- Limits login attempts, generation attempts, active work, previews (20), attempts (five per preview), source pages (four), output pages (one to eight), sessions (100 per preview / 2,000 total for the lifetime of the local state file, including operator preview sessions), response sizes, images, and stored asset size.
...
- Server-rendered public HTML is supported first. JavaScript-only catalogs, bot-protected pages, authentication, and nonstandard ports can fail with an actionable message; a source without a credible catalog becomes a one-page Home preview.
